...
Returns a 302 redirect to the target URL. The visit is recorded with timestamp, country (via IP geolocation), and user agent.
...
```json
{
  "short_code": "Xk9mR2q",
  "total_clicks": 42,
  "clicks_by_country": { "SG": 25, "US": 10, "DE": 7 },
...
**Title fetching with timeout** -- The target page's `<title>` tag is fetched synchronously during link creation with a 3-second connect / 5-second read timeout. If it fails for any reason, the link is still created with a nil title.
